      ...
      await collection.findOne({name:"monitor"})
                        .then(async obj=>{
                            if(obj!=null){ //objects exists??
                              ...
                              utility_object.sendEmail(process.env.USER_ENAIL,str);   //sending the data to the admins email address
                                 //   saveAgents(str,true);
                                },()=>{
...
function syncData(cnt){
  ...
  BehaviorExport('Agents successfully saved in the text file...');
    ...
    updateTextFile:async (collectionName,count)=>{ //sending back data to the requesst source and saving also in txt file incase the req fails
...
collection=db.collection('allAgents');
            ...
            try{
                          fs.readFile(`certusMealServer/files/${file_to_read_from}.txt`,'utf-8',(error,data)=>{
                            if(error){
...
let recently_saved_file='';

setInterval(()=>{
    let file_to_read_from=stateManager.get_current_excel_read_file();
    let current_admin_id=stateManager.get_admin_id();
    if(current_admin_id=='default' && file_to_read_from==undefined){ //no one is logged in yet
   //     console.log('No active Admin instance loaded....awaiting client login');
    }
    else if(current_admin_id!='default' && file_to_read_from==undefined){
        file_to_read_from='excel';
    }
    else if(file_to_read_from==recently_saved_file){return;} //avoiding loading the same instance repeatedly
    else{   //incase of a new instance
        recently_saved_file=file_to_read_from;
    inspector.start();
    inspector.readTextFile();
    inspector.analyze();
    }
},1000);
