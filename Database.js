...
let inspector={
    ...
    /*
    createAgent:(req,resp)=>{  //not used right now
        let paramsObj=req.body;
        if(paramsObj.ID!=DeviceID) return resp.status(402).json({message:"unknown device-id"});
    ...
    resetLunchState:async function(){ //not necessary 4 now
        let count=1;
    ...
    await collection.findOne({id2:count})  //testing using one item first
                .then(agent=>{
                    if(agent!=undefined){
                        console.log('Reseting agents list...');
                        let operation={
                            $set:{hadLunch:'No'},
                            $currentDate:{lastModified:true}
                        }

                        collection.updateMany({hadLunch:'yes'},operation)
                        .then(result=>{
                            console.log(`>>>>>>>>>>>>>modified count:${result.modifiedCount}`);
                             collection.updateOne({name:"monitor"},{$set:{did_reset_clients:true}})
                            result.modifiedCount?console.log(">>>>>>>>>>>>Agent state reset successful...")
                            :console.log(">>>>>>>>>>failed");
                        })
                    }
                },()=>{
                    console.log("Error while reading agents form the database...")
                  helper_functions. saveToLogFile("Error while reading agents form the database...");
                })
