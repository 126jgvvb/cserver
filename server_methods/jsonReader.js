...
let saveDataToJSON=require('../server_methods/jsonWriter');
const fs=require('fs');
...
class stateObject{
    ...
    get_current_excel_read_file(){
        return this.layoutObj.adminDetails.current_read_file;
    }
    ...
    load_stored_admins_into_memory(){
        ...
        fs.readFile(`certusMealServer/json/authorizedAdmins.json`,(err,data)=>{
            if(err){
                console.error(`Error reading from file:authorizedAdmins.json`,err);
                fileCreator('certusMealServer/json/authorizedAdmins.json');
                return false;
            }
            else{
                if(data!=undefined){
...
saveDataToJSON({fileName:`initialState-${obj.deviceID}`,obj:layoutObj})
    ...
    save_uploaded_file(obj){
        console.log(`saving changes to json file:${this.id}`)
    if(this.layoutObj.dynamicDiv.adminUploads.length!=0)    this.layoutObj.dynamicDiv.adminUploads.map(obj=>obj.status=false); //changing the default write/update file
        ...
        this.layoutObj.dynamicDiv.adminDetails.current_read_file=obj.fileName;
        saveDataToJSON({fileName:`initialState-${this.id}`,obj:this.layoutObj})
        return true;
    }
